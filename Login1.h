...
#pragma endregion
	private: System::Void btnSave_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		// Check if ID is numeric for trainee
		bool isNumeric = !idStr.empty() && std::all_of(idStr.begin(), idStr.end(), ::isdigit);

		if (isNumeric) {
			int numericId = std::stoi(idStr);
			if (ValidateTraineeLogin(numericId, password)) {
		...
		if (ValidateCoatchLogin(id, password)) {
		    ...
		    coachForm^ nextForm = gcnew coachForm(coach);
			nextForm->Location = this->Location;
			nextForm->Size = this->Size;
			nextForm->BackgroundImage = this->BackgroundImage;
			nextForm->BackgroundImageLayout = System::Windows::Forms::ImageLayout::Zoom;
	...
	private: bool ValidateTraineeLogin(int id, String^ password)
	{
		if (traineeList->find(id) != traineeList->end() &&
			traineeList->at(id).getPassword() == msclr::interop::marshal_as<std::string>(password)) {
			return true;
		}
		return false;
	}
	...
	{
		std::string coachId = msclr::interop::marshal_as<std::string>(id);
		if (coatchList->find(coachId) != coatchList->end() &&
			coatchList->at(coachId).getpassword() == msclr::interop::marshal_as<std::string>(password)) {
			return true;
		}
		return false;
	}
	...
	{
		std::string recId = msclr::interop::marshal_as<std::string>(id);
		if (receptionistList->find(recId) != receptionistList->end() &&
			receptionistList->at(recId).getpassword() == msclr::interop::marshal_as<std::string>(password)) {
			return true;
		}
		return false;
	}

	private: bool ValidateManagerLogin(String^ id, String^ password)
	{
		std::string manId = msclr::interop::marshal_as<std::string>(id);
		if (manager->find(manId) != manager->end() &&
			manager->at(manId).getpassword() == msclr::interop::marshal_as<std::string>(password)) {
			return true;
		}
		return false;
	}
